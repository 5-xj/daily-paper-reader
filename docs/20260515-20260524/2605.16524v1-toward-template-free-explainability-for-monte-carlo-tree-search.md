...
tags: ["query:ad"]
score: 8.0
evidence: 基于大语言模型的MCTS启发式搜索可解释性
tldr: MCTS等概率搜索算法难以解释。现有方法依赖手工约束，无法适应问题变化。本文提出无需模板的框架，利用大语言模型从搜索轨迹端到端生成解释。通过映射问题意图、评估证据、按需扩展树，并利用访问计数等统计信息生成解释。实验首次证明LLM可直接作为概率搜索的解释器，无需中间形式表示。
source: arxiv
...
motivation: 现有MCTS解释方法需要手工逻辑约束，缺乏通用性；LLM无需模板即可生成解释。
method: 框架将自然语言问题映射到意图类别，检查证据充分性，必要时触发树扩展，利用访问计数、价值估计等生成解释。
result: 实验首次验证LLM可作为概率搜索的端到端解释器，无需中间形式表示。
conclusion: LLM能有效解释MCTS决策，减少手工工作，提高可解释性。
---

## 摘要
概率搜索算法（例如蒙特卡洛树搜索，MCTS）在解决不确定性下的序贯决策任务方面已被证明非常有效。然而，对于仅基于原始树统计数据的最终用户而言，解释结合了基于置信区间上界（bandit）的树遍历和基于模拟的值估计的非对称搜索树是困难的。尽管先前的工作需要手动设计的正式逻辑约束，这些约束在问题变化时必须更新，但我们提出了一个框架，使得大型语言模型（LLMs）能够以端到端的方式从记录的搜索轨迹中生成基于证据的MCTS决策解释。我们的框架将自然语言问题映射到一组结构化的意图类别，确定现有树是否包含足够的证据，在需要时触发有针对性的扩展，并使用树统计数据（如访问次数、值估计和风险信息）生成解释。实验结果首次证明，LLMs可以作为概率搜索的端到端解释器，而无需中间形式化表示。
