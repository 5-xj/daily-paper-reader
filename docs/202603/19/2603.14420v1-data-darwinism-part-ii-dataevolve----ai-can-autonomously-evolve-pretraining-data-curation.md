...
tags: ["query:llm-ec"]
score: 7.0
evidence: 通过迭代优化实现策略自动演化的闭环演化框架
tldr: 本研究提出 DataEvolve 框架，旨在解决大规模预训练语料库中手动设计数据清洗策略效率低下的问题。该框架通过闭环进化机制，自动识别数据质量问题并迭代优化处理策略。实验在 672B token 的 Nemotron-CC 数据集上生成了 Darwin-CC，训练出的 3B 模型在多个基准测试中超越了 DCLM 和 FineWeb-Edu 等主流数据集，证明了 AI 自主进化数据策展策略的可行性与优越性。
selection_source: fresh_fetch
motivation: 针对大规模异构预训练数据，手动设计针对性的清洗策略成本极高且难以扩展。
method: 提出 DataEvolve 框架，通过“识别问题-生成策略-执行评估-迭代优化”的闭环进化过程自动演进数据处理方案。
result: 基于该框架生成的 Darwin-CC 数据集在 18 项基准测试中表现优异，显著超越了 DCLM、Ultra-FineWeb 和 FineWeb-Edu。
conclusion: 实验证明迭代进化是实现预训练规模数据策展的关键，AI 自主生成的策略能有效提升模型在知识密集型任务中的表现。
---

## 摘要
数据达尔文主义（第一部分）建立了数据处理的十级层次结构，表明更强的处理能力可以释放更大的数据价值。然而，该工作依赖于针对单一类别的手动设计策略。现代预训练语料库包含跨越不同领域和内容类型的数百个异构类别，每个类别都需要专门的处理。在这种规模下，手动设计策略变得极其困难且成本高昂。这提出了一个关键问题：策略能否以自动化的方式演化？我们介绍了 DataEvolve，这是一个使策略能够通过迭代优化而非手动设计进行演化的框架。对于每个数据类别，DataEvolve 在一个封闭的演化循环中运行：它识别质量问题，生成候选策略，在采样数据上执行这些策略，评估结果，并跨代改进方法。该过程通过发现问题的经验池和跟踪跨迭代性能的策略池来积累知识。应用于来自 Nemotron-CC 的 8 个类别（涵盖 672B token），DataEvolve 产生了 Darwin-CC，这是一个包含 504B token 的数据集，其策略经过每个类别 30 次迭代演化而成。在 500B token 上训练 3B 模型，Darwin-CC 的表现优于原始数据（+3.96 分），并在 18 个基准测试中实现了 44.13 的平均分，超过了 DCLM、Ultra-FineWeb 和 FineWeb-Edu，在 MMLU 等知识密集型任务上取得了显著提升。分析表明，演化出的策略收敛于以清洗为中心的方法：有针对性的噪声去除和具有领域感知保留的格式规范化，呼应了第一部分中的 L4（生成式精炼）原则。消融研究证实迭代演化至关重要：优化后的策略比次优策略高出 2.93 分，确立了演化策略设计对于预训练规模的数据策展是可行且必要的。
