...
tags: ["query:llm-ec"]
score: 6.0
evidence: 通过挖掘仓库自动获取智能体技能
tldr: 本研究针对大语言模型在自主工作流中缺乏专业程序性知识的问题，提出了一种从GitHub等开源仓库自动挖掘并提取智能体技能的系统框架。该框架通过结构分析、语义识别及标准化转换，从TheoremExplainAgent等先进系统中提取可视化与教育能力。这种方法无需重新训练模型即可扩展LLM能力，并结合安全治理与多维评估，实现了程序性知识的大规模获取与应用。
selection_source: fresh_fetch
motivation: 旨在解决通用大语言模型在处理复杂自主工作流时，因缺乏特定领域程序性专业知识而导致的效能受限问题。
method: 开发了一套包含仓库结构分析、基于稠密检索的语义技能识别以及SKILL.md标准化格式转换的自动化提取框架。
result: "实验表明，该框架提取的教育类技能在知识传递效率上提升了40%，且教学质量可媲美人工编写的教程。"
conclusion: 通过大规模挖掘开源智能体仓库，可以实现程序性知识的可持续扩展，在不重训模型的情况下显著增强LLM的实际应用能力。
---
