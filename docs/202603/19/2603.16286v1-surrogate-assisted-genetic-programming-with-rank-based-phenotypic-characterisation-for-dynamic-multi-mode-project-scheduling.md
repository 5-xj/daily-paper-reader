...
## 摘要
动态多模式资源受限项目调度问题（DMRCPSP）具有重要的实际意义，因为它要求在不断变化的项目状态和资源可用性下做出实时决策。遗传编程（GP）已被证明能有效地为这类决策任务演化出启发式规则；然而，演化过程通常依赖于大量基于模拟的适应度评估，导致计算成本高昂。代理模型为降低评估成本提供了一种极具前景的解决方案，但其在 GP 中的应用需要针对特定问题的启发式规则表型表征（PC）方案。目前，应用于 DMRCPSP 的 GP 尚缺乏合适的 PC 方案。本文提出了一种基于排名的 PC 方案，该方案源于决策情境中由启发式驱动的可选“活动-模式”对和活动组的排序。生成的 PC 向量使代理模型能够估计未评估 GP 个体的适应度。基于该方案，开发了一种代理辅助的 GP 算法。实验结果表明，与现有的 DMRCPSP 领域最先进的 GP 方法相比，所提出的代理辅助 GP 能够始终更早地识别出高质量的启发式规则，且仅引入了极小的计算开销。进一步的分析表明，代理模型为后代选择提供了有用的指导，从而提高了演化效率。
