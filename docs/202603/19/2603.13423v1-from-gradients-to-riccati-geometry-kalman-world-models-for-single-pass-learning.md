...
title_zh: 从梯度到黎卡提几何：用于单次学习的卡尔曼世界模型
authors: Andrew Kiruluta
date: 2026-03-13
pdf: "https://arxiv.org/pdf/2603.13423v1"
tags: ["query:ahd"]
score: 6.0
evidence: 基于控制理论的 LLM 无梯度训练与自适应范式
tldr: 本研究挑战了反向传播在机器学习中的主导地位，提出了卡尔曼世界模型（KWM）。该模型利用递归贝叶斯滤波而非自动微分进行训练，将参数学习转化为卡尔曼增益自适应。通过将该框架扩展至Transformer架构，将内部激活视为潜在动态状态进行修正，实现了基于控制理论的无梯度训练范式。KWM在序列建模任务中表现出与梯度下降相当的性能，并具备更强的鲁棒性和持续学习能力。
selection_source: fresh_fetch
motivation: 旨在探索除反向传播之外的优化动力系统的方法，以实现更高效的在线学习和单次通过学习。
method: 提出卡尔曼世界模型，利用递归贝叶斯滤波和卡尔曼增益自适应代替梯度下降，并将其应用于Transformer架构。
result: 在序列建模任务中取得了具有竞争力的性能，并在鲁棒性和持续适应性方面表现优异。
conclusion: 基于控制理论的无梯度训练范式为大型语言模型和动力系统的优化提供了一种稳定且高效的新途径。
---

## 摘要
反向传播在现代机器学习中占据主导地位，但它并非优化动力系统的唯一原则性方法。我们提出了卡尔曼世界模型（KWM），这是一类通过递归贝叶斯滤波而非反向模式自动微分进行训练的学习型状态空间模型。我们用卡尔曼式增益自适应取代了参数学习，而非采用梯度下降更新。训练过程转变为在线滤波；误差信号转变为新息（innovations）。我们进一步将该框架扩展到基于 Transformer 的大语言模型（LLM），其中内部激活被视为通过新息项进行修正的潜动态状态。这产生了一种基于控制理论的无梯度训练与自适应范式。我们推导了稳定性条件，分析了计算复杂度，并提供了在序列建模任务上的实证结果，展示了具有竞争力的性能以及更强的鲁棒性和持续自适应特性。
