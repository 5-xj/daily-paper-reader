...
title_zh: 通过符号递归自对齐利用经验证的推理稳定迭代自训练
authors: Xinyu Zhang
date: 2026-03-23
...
tldr: 针对迭代自训练中推理错误累积导致的“递归漂移”问题，本文提出神经符号递归自对齐（NSRSA）框架。该方法通过嵌入符号验证子系统，在推理步骤层面过滤低质量数据，利用sympy验证算术运算并检查逻辑一致性。实验表明，NSRSA能有效剔除“侥幸猜对”的错误推理，结合DPO进一步提升了模型区分正确与错误推理的能力，为实现稳定、可衡量的递归自我提升提供了新路径。
source: arxiv
...
motivation: 迭代自训练中模型生成的推理错误会随迭代累积，导致性能退化和模式崩溃。
method: 提出NSRSA框架，利用符号验证系统在推理步骤级别对生成数据进行质量把控，验证算术准确性与逻辑一致性。
result: "NSRSA在GSM8K任务中剔除了约34%虽答案正确但推理错误的样本，并通过DPO显著提升了模型的奖励准确度。"
conclusion: 引入外部符号验证能有效稳定递归自改进过程，使模型在具备自动化验证条件的领域内实现可靠的能力增长。
---

## 摘要
递归自我提升——即模型在其自身输出上进行迭代训练——有望实现持续的能力增长，但面临着一个根本性障碍：递归漂移。随着模型在多次迭代中基于自生成数据进行训练，中间推理过程中的错误会不断累积，导致模式崩溃和性能下降。我们提出了神经符号递归自对齐（NSRSA），通过嵌入一个符号验证子系统，在推理步骤层面把控训练数据质量，从而稳定迭代自训练。与仅针对结果的过滤（这种方式会接纳推理有误但“侥幸猜对”的结果）不同，NSRSA 通过 sympy 验证每个算术运算，检查推理步骤间的逻辑流一致性，并强制执行领域约束。我们在 GSM8K 数据集上使用 Qwen3-4B-Thinking 模型，在五种条件下进行了 5 次自训练迭代评估：无验证、结果验证、多数投票、完整的 NSRSA 符号验证以及结合 DPO 的 NSRSA。我们的过滤分析表明，NSRSA 拒绝了约 34% 通过结果验证但答案正确的解法，从而从训练集中排除了推理有误的“侥幸猜对”样本。我们进一步证明，利用 NSRSA 验证构建 DPO 偏好对，可以教会模型区分正确与错误的推理（奖励准确率从 46% 提升至 63%）。NSRSA 提供了一个可扩展的框架，展示了在可进行自动验证的领域中，外部符号验证如何使递归自我提升变得可衡量且可靠。
