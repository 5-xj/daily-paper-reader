...
tags: ["query:ahd"]
score: 6.0
evidence: 从组合搜索空间中发现帕累托最优配置
tldr: 针对复合AI系统在固定算力下难以应对动态负载的问题，本文提出Compass框架。该框架通过离线优化和在线适配实现配置的动态切换。其核心包括用于发现帕累托最优配置的COMPASS-V算法、基于排队论制定切换策略的Planner，以及根据实时负载调整配置的Elastico控制器。实验表明，Compass在保证高SLO达标率的同时，显著降低了配置搜索开销，并实现了精度与延迟的动态平衡。
selection_source: fresh_fetch
motivation: 复合AI系统在固定基础设施下难以兼顾精度、延迟和成本，且现有优化方法缺乏对动态负载变化的适应性。
method: 提出Compass框架，包含COMPASS-V搜索算法、基于排队论的策略规划器以及实时监控并切换配置的Elastico控制器。
result: "COMPASS-V在减少57.5%评估次数的同时保持100%召回率，且运行时SLO达标率达到90-98%，远超静态基准。"
conclusion: Compass通过动态配置切换有效解决了复合AI系统在资源受限环境下的性能权衡问题，实现了高效的负载自适应。
---
