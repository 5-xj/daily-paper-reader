...
tldr: 针对大规模稀疏多目标优化问题（LSMOPs）中高维空间与稀疏解导致的探索平衡难题，本文提出一种结合概率退火机制的进化算法。该算法通过收敛导向和退火两种不同熵特性的概率向量，实现了从全局探索到局部精细化搜索的动态平衡，显著提升了在有限评估次数下识别关键非零变量的能力。
selection_source: fresh_fetch
motivation: 大规模稀疏优化问题中高维决策空间与稀疏性的共存，使得现有算法在平衡探索与开发以及识别关键变量方面面临巨大挑战。
method: 提出一种利用双概率向量驱动的进化算法，结合低熵收敛向量的稳定开发能力与退火向量的自适应探索机制。
result: 在基准测试和实际应用中的实验证明，该算法在收敛性和解的多样性方面均优于当前最先进的进化算法。
conclusion: 通过引入概率退火机制，该研究为解决大规模稀疏多目标优化中的探索与开发冲突提供了高效且稳健的解决方案。
---

## 摘要
大规模稀疏多目标优化问题（LSMOPs）在现实应用中非常普遍，其最优解通常仅包含少量非零变量，例如对抗攻击、关键节点检测和稀疏信号重构。由于 LSMOPs 的函数评估通常依赖于涉及大量决策变量的大规模数据集，搜索空间变得极高维。稀疏性与高维性的共存极大地加剧了探索（exploration）与利用（exploitation）之间的冲突，使得现有的多目标演化算法（MOEAs）难以在有限的函数评估次数内识别出关键的非零决策变量。为了应对这一挑战，本文提出了一种用于大规模稀疏多目标优化的概率退火演化算法。该算法由两个具有不同熵特征的概率向量驱动：一个具有较低熵的收敛导向概率向量确保了稳定的利用，而一个熵逐渐降低的退火概率向量则实现了从全局探索到局部精细化的自适应过渡。通过整合这些互补的搜索动态，所提算法在探索与利用之间实现了动态平衡。在基准问题和实际应用上的实验结果表明，所提算法在收敛性和多样性方面均优于目前最先进的演化算法。
