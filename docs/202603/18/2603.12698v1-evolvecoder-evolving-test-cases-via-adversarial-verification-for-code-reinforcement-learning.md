...
tldr: 针对代码生成任务中强化学习受限于验证信号弱且静态的问题，本文提出了EvolveCoder框架。该框架通过对抗性验证机制，根据候选解的执行行为迭代优化测试用例，提升其难度和区分度。基于此构建了EvolveCoder-22k数据集，实验证明该方法能显著增强验证强度，并在多个基准测试中提升了模型性能，为代码强化学习提供了更有效的信号。
selection_source: fresh_fetch
motivation: 现有的代码强化学习数据集验证信号较弱且固定，限制了模型通过可验证奖励进行优化的效果。
method: 提出一种基于解条件的对抗性验证框架，通过多轮迭代演化测试用例，以增加难度、提高区分度并减少冗余。
result: 构建了EvolveCoder-22k数据集，使Qwen3-4B在四个基准测试上平均提升4.2分，显著优于同规模基准模型。
conclusion: 对抗性且基于解条件的验证机制对于实现高效且可扩展的代码生成强化学习至关重要。
---
