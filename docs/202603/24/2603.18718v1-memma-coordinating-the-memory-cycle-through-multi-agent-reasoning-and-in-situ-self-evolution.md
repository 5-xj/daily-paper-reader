...
tldr: 针对现有大模型智能体在外部存储管理中存在的构建、检索与利用环节脱节，以及缺乏全局策略指导和反馈机制的问题，本文提出了MemMA框架。该框架采用多智能体协作模式，在前向路径通过Meta-Thinker提供结构化策略指导，在后向路径引入原位自进化机制，通过合成探针问答对进行自我验证与修复。实验证明，MemMA能显著提升长程交互性能，且具有良好的即插即用兼容性。
selection_source: fresh_fetch
motivation: 现有存储增强型智能体将存储的构建、检索和利用视为孤立子任务，导致缺乏全局策略指导且难以从下游失败中学习修复。
method: 提出MemMA框架，通过Meta-Thinker协调前向路径的存储管理与检索，并利用原位自进化机制在后向路径实现存储的自我验证与修复。
result: 在LoCoMo基准测试中，MemMA在多种大模型底座和存储后端上均表现出优于现有基准的效果。
conclusion: MemMA通过多智能体协同和闭环反馈机制，有效解决了长程交互中的存储管理难题，为构建更可靠的记忆增强型智能体提供了新思路。
---
