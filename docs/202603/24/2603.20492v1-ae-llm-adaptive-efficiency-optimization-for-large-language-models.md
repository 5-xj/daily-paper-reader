...
motivation: 现有的单一效率优化技术在不同任务和硬件约束下表现不一，缺乏能根据具体场景自动组合最优策略的统一框架。
method: 提出一种多目标优化框架和高效搜索算法，在架构、微调及推理阶段自动识别精度与资源消耗之间的帕累托最优配置。
result: "在15个模型和10个任务上，AE-LLM在精度损失仅1.2%的情况下实现了平均2.8倍的效率提升，并具有良好的跨模态泛化性。"
conclusion: AE-LLM为大模型的高效部署提供了一个强大的自动化工具，有效解决了精度、延迟、内存和能耗之间的复杂权衡问题。
---
