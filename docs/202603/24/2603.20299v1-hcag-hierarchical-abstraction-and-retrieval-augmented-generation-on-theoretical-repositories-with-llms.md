...
tldr: 针对复杂理论驱动型代码库（如算法博弈论）中抽象概念与具体实现之间的语义鸿沟，本文提出HCAG框架。该框架通过离线构建连接理论、架构与实现的层次化知识库，并结合在线自顶向下的层次化检索与脚手架生成，将仓库级代码生成转化为结构化规划过程。实验证明，HCAG在代码质量和架构一致性上显著优于现有RAG方法，并能通过生成的对齐数据集提升领域大模型的性能。
selection_source: fresh_fetch
motivation: 现有的代码RAG方法难以捕捉复杂理论驱动型代码库中的高层架构模式和跨文件依赖关系。
method: 提出HCAG框架，通过离线层次化抽象构建多分辨率知识库，并结合在线自顶向下的检索与多智能体协作生成。
result: 实验表明HCAG在代码质量、架构连贯性和需求通过率方面显著优于现有的仓库级生成方法。
conclusion: HCAG为从结构化代码库中挖掘、重用和生成复杂系统提供了一种通用的层次化抽象与检索范式。
---
