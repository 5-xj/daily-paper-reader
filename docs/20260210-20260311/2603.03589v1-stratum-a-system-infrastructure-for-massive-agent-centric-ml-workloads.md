...
tags: ["query:ahd"]
score: 7.0
evidence: 用于智能体流水线搜索与优化的基础设施
tldr: 随着大语言模型的发展，由智能体自主生成和优化机器学习流水线的需求激增，但现有的 Python 生态系统在处理此类大规模、探索性任务时存在性能瓶颈。本文提出 stratum 系统基础设施，通过解耦执行与推理、将流水线批量编译为优化图以及引入 Rust 高性能运行时，实现了与现有 Python 库的无缝集成。该系统显著提升了智能体驱动的流水线搜索效率，实验显示其加速比高达 16.6 倍，为未来自动化机器学习系统提供了关键支撑。
selection_source: fresh_fetch
motivation: 现有的 Python ML 生态系统是为人类交互设计的，难以高效支持由 LLM 智能体驱动的大规模、高并发且具有高度探索性的机器学习流水线搜索任务。
method: 提出了 stratum 架构，通过将流水线执行与规划推理分离，将批量流水线编译为优化执行图，并利用 Rust 编写的高性能运行时在异构后端高效执行。
result: 初步实验结果显示，stratum 在处理大规模智能体流水线搜索任务时，相比传统方法可实现高达 16.6 倍的性能提升。
conclusion: stratum 成功解决了智能体中心化 ML 工作负载与现有基础设施之间的不匹配问题，为高效、可扩展的自动化机器学习探索奠定了基础。
---
