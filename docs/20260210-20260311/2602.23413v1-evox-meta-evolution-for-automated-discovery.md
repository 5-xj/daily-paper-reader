...
## 摘要
最近的研究（如 AlphaEvolve）表明，将大语言模型（LLM）驱动的优化与演化搜索相结合，可以有效改进跨领域的程序、提示词和算法。在这种范式中，先前评估过的解被重新利用，以引导模型生成新的候选解。至关重要的是，这种演化过程的有效性取决于搜索策略：即如何选择和变异先前的解以生成新的候选解。然而，大多数现有方法依赖于具有预定义参数（例如探索-利用比例）的固定搜索策略，这些参数在整个执行过程中保持不变。虽然在某些设置下有效，但这些方法往往无法适应不同的任务，甚至在同一任务中，随着搜索空间随时间变化也难以自适应。我们介绍了 EvoX，一种优化自身演化过程的自适应演化方法。EvoX 联合演化候选解及其生成策略，并根据进展持续更新先前解的选择和变异方式。这使得系统能够在优化过程中在不同的搜索策略之间动态切换。在近 200 个真实世界的优化任务中，EvoX 在大多数任务上的表现优于现有的 AI 驱动演化方法，包括 AlphaEvolve、OpenEvolve、GEPA 和 ShinkaEvolve。
