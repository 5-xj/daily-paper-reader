...
motivation: 传统的查询重写规则自动发现方法在面对复杂查询计划时，因搜索空间巨大和严重冗余而难以扩展。
method: SLER利用标准化模板减少结构冗余，并引入排序学习模型来预筛选最有潜力的模板对，从而引导高效的规则枚举。
result: 在超过1.1万个真实SQL查询的测试中，SLER自动构建了拥有100多万条规则的库，支持深度复杂的查询模式转换。
conclusion: SLER的模板驱动设计和学习排名机制具有良好的可扩展性，能够高效发现复杂的重写规则，显著提升了数据库优化器的潜力。
---
