---
title: A universal LLM Framework for General Query Refinements
title_zh: 一种用于通用查询精炼的通用 LLM 框架
authors: "Eldar Hacohen, Yuval Moskovitch, Amit Somech"
date: 2026-02-17
pdf: "https://arxiv.org/pdf/2602.15681v1"
tags: ["query:ahd"]
score: 6.0
evidence: 基于LLM提示优化的查询改进框架
tldr: 针对现有SQL查询重写工具仅支持特定查询或约束类型的局限性，本文提出了通用框架OmniTune。该框架利用大语言模型的提示优化（OPRO）技术，通过两阶段方案探索重写子空间并采样候选方案，并结合历史记录和天际线摘要提供有效反馈。实验证明，OmniTune不仅能处理传统重写任务，还能应对现有方案无法解决的复杂场景，在通用性上取得了显著突破。
selection_source: fresh_fetch
motivation: 现有的SQL查询重写研究通常局限于特定的查询类别或约束条件，缺乏处理复杂通用场景的能力。
method: 提出OmniTune框架，采用基于大语言模型的两步提示优化方案，结合历史摘要和天际线反馈来探索并采样最优查询。
result: 在综合基准测试中，OmniTune成功处理了传统任务以及现有解决方案无法涵盖的更复杂重写场景。
conclusion: OmniTune证明了利用大语言模型进行提示优化是解决通用SQL查询重写问题的有效且灵活的途径。
---

## 摘要
许多研究已经探讨了 SQL 查询精炼问题，其目标是对输入查询进行最小化修改，使其满足一组指定的约束条件。然而，这些工作通常针对受限类别的查询或约束。我们提出了 OmniTune，这是一个利用基于大语言模型（LLM）的提示优化（OPRO）来精炼任意 SQL 查询的通用框架。OmniTune 采用两步式 OPRO 方案，探索具有前景的精炼子空间并在其中采样候选方案，并辅以简洁的历史记录和天际线（skyline）摘要以提供有效的反馈。在综合基准测试上的实验表明，OmniTune 既能处理先前研究过的精炼任务，也能处理超出现有解决方案范围的更复杂场景。

## Abstract
Numerous studies have explored the SQL query refinement problem, where the objective is to minimally modify an input query so that it satisfies a specified set of constraints. However, these works typically target restricted classes of queries or constraints. We present OmniTune, a general framework for refining arbitrary SQL queries using LLM-based optimization by prompting (OPRO). OmniTune employs a two-step OPRO scheme that explores promising refinement subspaces and samples candidates within them, supported by concise history and skyline summaries for effective feedback.   Experiments on a comprehensive benchmark demonstrate that OmniTune handles both previously studied refinement tasks and more complex scenarios beyond the scope of existing solutions.