...
method: 提出由离线智能体（基于代理指标生成假设）和在线智能体（基于核心业务指标验证方案）组成的端到端自动化工作流。
...
## 摘要
优化大规模机器学习系统（例如全球视频平台的推荐模型）需要探索巨大的超参数搜索空间，更关键的是，需要设计复杂的优化器、架构和奖励函数，以捕捉细微的用户行为。在这些领域取得实质性改进是一项极具挑战性的任务，传统上依赖于大量的迭代手动测试新假设。我们提出了一种自演化系统，利用大语言模型（LLM），特别是 Google 的 Gemini 系列，在端到端自动化工作流中自主生成、训练和部署高性能且复杂的模型变更。该自演化系统由一个离线 Agent（内环）和一个在线 Agent（外环）组成。离线 Agent 使用代理指标进行高吞吐量的假设生成，而在线 Agent 则在生产环境中根据滞后的北极星业务指标验证候选方案。我们的 Agent 充当专业的机器学习工程师（MLE）：它们展现出深度的推理能力，能够发现优化算法和模型架构中的新颖改进，并制定针对长期用户参与度的创新奖励函数。该方法的有效性已通过在 YouTube 的多次成功生产发布得到证实，证明了自主的 LLM 驱动演化在开发速度和模型性能方面均能超越传统的工程工作流。
