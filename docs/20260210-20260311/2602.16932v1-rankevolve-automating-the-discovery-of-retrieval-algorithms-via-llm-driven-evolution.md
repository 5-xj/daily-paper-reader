...
tldr: 本研究针对传统词法检索算法（如BM25）过度依赖人工设计的问题，提出了RankEvolve框架。该框架结合大语言模型与进化搜索，将检索算法表示为可执行代码，通过迭代变异、重组和基于12个数据集的性能筛选，自动探索更优的排序逻辑。实验表明，RankEvolve从基础算法出发，成功发现了多种新颖且高效的检索算法，在BEIR、BRIGHT及TREC DL等基准测试中展现了极强的泛化能力，为自动化算法发现提供了新范式。
selection_source: fresh_fetch
motivation: 传统词法检索算法主要依赖人工直觉和参数微调，限制了更高效排序逻辑的自动化发现。
method: 提出RankEvolve框架，利用LLM驱动的程序进化，通过对检索算法代码进行迭代变异和选择来发现新算法。
result: 进化出的新算法在BEIR、BRIGHT和TREC DL等多个基准测试中均表现出优于基准算法的性能和泛化性。
conclusion: 研究证明了评估器引导的LLM程序进化是自动发现新型高效排序算法的一种实用且有效的路径。
---
