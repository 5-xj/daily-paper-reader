...
tldr: 本研究针对大语言模型在推理阶段适应多样化任务的需求，提出了MASS元学习框架。该框架允许模型在测试时通过生成特定问题的合成训练数据并进行针对性自我更新来实现自我进化。通过双层优化机制，内循环在合成数据上进行微调，外循环利用元梯度优化数据生成质量，从而在数学推理等任务中实现了高效且数据节约的测试时自适应。
selection_source: fresh_fetch
motivation: 旨在提升大语言模型在推理阶段针对特定领域和任务进行实时自适应与自我改进的能力。
method: 提出MASS框架，利用双层优化和元梯度技术，通过内循环合成数据微调与外循环性能奖励来优化模型更新。
result: 在数学推理实验中，MASS成功学习到针对每个实例的课程学习策略，实现了高效且数据节约的测试时自适应。
