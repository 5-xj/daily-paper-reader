...
tldr: 本研究探讨了利用图神经网络（GNN）预测CDCL SAT求解器的初始分支顺序，以提高求解效率。通过提出三种可扩展的标签生成方法，训练GNN在求解前对变量进行排序。实验表明，该方法在随机3-CNF和伪工业基准测试中显著提升了求解速度，并展现出良好的泛化能力，但在处理复杂工业实例时仍面临动态启发式算法覆盖和预测难度大的挑战。
selection_source: fresh_fetch
motivation: 寻找高效的分支顺序对提升SAT求解性能至关重要，但预先确定最优顺序具有极大挑战性。
method: 提出利用图神经网络结合三种高效的标签生成方法，在求解前预测并初始化CDCL求解器的变量分支顺序。
result: 该方法在随机和伪工业实例上实现了显著加速，并能泛化至比训练集规模大得多的问题。
conclusion: 虽然初始排序对特定类型实例有效，但对于复杂工业实例，求解器的动态启发式机制往往会迅速覆盖初始设置，限制了性能提升。
---
