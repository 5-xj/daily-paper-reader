...
tldr: "本研究针对过度参数化神经网络的计算成本问题，基于强彩票假设（SLT），提出了一种利用连续松弛伯努利门控发现稀疏子网络的新方法。该方法在保持初始权重冻结的情况下，通过全微分的端到端优化来训练门控参数，避免了传统方法中不可微的梯度估计。实验证明，该方法在CNN和ViT等多种架构上能以极小的精度损失实现高达90%的稀疏度，显著优于现有技术，为网络压缩提供了高效的可扩展方案。"
selection_source: fresh_fetch
motivation: 现有的强彩票子网络发现方法主要依赖不可微的评分选择机制，导致优化效率低下且难以在复杂模型中扩展。
method: 提出使用连续松弛的伯努利门控，通过全微分的端到端优化和L0正则化，在冻结权重的随机初始化网络中筛选子网络。
result: "在ResNet和Vision Transformer等多种架构上实现了高达90%的稀疏度，且在同等精度下其稀疏能力接近主流edge-popup方法的两倍。"
conclusion: 该方法建立了首个无需直通估计器（STE）的全微分SLT发现框架，为预训练网络的高效稀疏化提供了可扩展的解决方案。
---

## 摘要
过参数化的神经网络在资源受限的部署中会产生极高的内存和计算成本。强彩票（SLT）假设表明，随机初始化的网络包含稀疏子网络，无需权重训练即可达到具有竞争力的准确率。现有的 SLT 方法（特别是 edge-popup）依赖于不可微的基于分数的选择，限制了优化效率和可扩展性。我们提出使用连续松弛的伯努利门控，通过全可微的端到端优化来发现 SLT——即仅训练门控参数，同时保持所有网络权重冻结在其初始值。连续松弛实现了对 $\ell_0$ 正则化目标的直接基于梯度的优化，消除了对不可微梯度估计器或迭代剪枝周期的需求。据我们所知，这是首个用于 SLT 发现的全可微方法，避免了直通估计器（straight-through estimator）近似。在全连接网络、CNN（ResNet、Wide-ResNet）和 Vision Transformers（ViT、Swin-T）上的实验表明，在准确率损失极小的情况下，稀疏度可达 90%——在同等准确率下，稀疏度几乎是 edge-popup 的两倍——这为预训练网络稀疏化建立了一个可扩展的框架。
