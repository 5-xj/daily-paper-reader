...
## 摘要
为流处理系统配置高效性能，特别是在云原生部署中，是一项具有挑战性且很大程度上依赖人工的任务。我们提出了一种实验驱动的自动化配置优化方法，该方法结合了三个阶段：用于初始探索的拉丁超立方抽样（Latin Hypercube Sampling）、用于引导式随机搜索的模拟退火（Simulated Annealing）以及用于局部细化的爬山算法（Hill Climbing）。该工作流与云原生基准测试框架 Theodolite 集成，实现了 Kubernetes 上的自动化实验编排以及对性能不佳配置的早期终止。在使用 Kafka Streams 和基于 Kubernetes 的云测试平台进行的实验评估中，我们的方法识别出的配置比默认配置提高了高达 23% 的吞吐量。结果表明，带有早期终止的拉丁超立方抽样和模拟退火在探索配置空间方面特别有效，而通过爬山算法进行的额外微调带来的收益有限。
